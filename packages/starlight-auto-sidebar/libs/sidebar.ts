...
import { getDefaultLang, getEntryOrder, getEntryPrevNextLinks, type EntryData } from './starlight'
...
export async function updatePageSidebar(
  ...
  return updateSidebarItems(items, metadata, context)
}
  ...
  for (const [index, itemConfig] of context.sidebar.entries()) {
    ...
    if (!item) {
      ...
      const {
        ...
        prev,
        next,
      } = await updateSidebarItems(item.entries, metadata, { ...context, sidebar: itemConfig.items })
      ...
      // Sorting needs to be done before computing the prev/next links.
...
async function updateAutogeneratedGroup(
  ...
  result.sidebar = await Promise.all(
    group.entries.map(async (item, index) => {
      ...
      if (item.isCurrent) {
        const { prev, next } = await getEntryPrevNextLinks(getSidebarLinkId(item))
  ...
  return Promise.all(
    items.map(async (item, index) => {
      if (isSidebarLink(item)) {
        ...
        return { index, order: await getEntryOrder(id), path: id }
      }
...
function getAutogeneratedGroupDirMetadata(context: SidebarUpdateContext) {
  ...
  return metadata[
    isAutogeneratedGroupRootSegment(context)
