  ...
  result.sidebar = await Promise.all(
    group.entries.map(async (item, index) => {
      if (isSidebarGroup(item)) {
        ...
        const collapsed = metadata?.collapsed ?? item.collapsed
...
function getAutogeneratedGroupItemsOrder(
  ...
  return Promise.all(
    items.map(async (item, index) => {
      ...
      if (metadata?.hidden) order.hidden = true
...
function getAutogeneratedGroupDirMetadata(context: SidebarUpdateContext) {
  ...
  if (!context.isMultilingual || !context.locale) return metadata[id]

  return metadata[`${context.locale}/${id}`] ?? metadata[DefaultLocale ? `${DefaultLocale}/${id}` : id]
}
