...
import { DefaultLocale, getDefaultLang, getEntryOrder, type Locale } from './starlight'
...
export async function updatePageSidebar(
  ...
  locale: Locale,
  data: Pick<StarlightRouteData['entry']['data'], 'next' | 'prev'>,
  context: StarlightAutoSidebarContext,
) {
  const previousLinks = getCurrentSidebarLinks(items)
  const result = await updateSidebarItems(items, metadata, locale, context, [createUpdateContextRootSegment(items)])

  updateResultWithPrevNextLinks(result, previousLinks, data, context)
...
function updateResultWithPrevNextLinks(
  result: SidebarUpdateResult,
  previousLinks: ReturnType<typeof getCurrentSidebarLinks>,
  data: Pick<StarlightRouteData['entry']['data'], 'next' | 'prev'>,
  context: StarlightAutoSidebarContext,
) {
  if (context.pagination === false) return

  const currentLinks = getCurrentSidebarLinks(result.sidebar)
  if (!previousLinks || !currentLinks) return

  const isDifferentCurrentLink = previousLinks.current !== currentLinks.current

  if (isDifferentCurrentLink || previousLinks.prev !== currentLinks.prev) {
    result.prev = currentLinks.prev ?? null
    updateResultWithEntryPrevNextLink(result, 'prev', data.prev)
  }

  if (isDifferentCurrentLink || previousLinks.next !== currentLinks.next) {
    result.next = currentLinks.next ?? null
    updateResultWithEntryPrevNextLink(result, 'next', data.next)
  }
}
...
function updateResultWithEntryPrevNextLink(
  result: SidebarUpdateResult,
  type: 'prev' | 'next',
  entryPrevNextLink: StarlightRouteData['entry']['data']['prev'],
) {
...
function getCurrentSidebarLinks(items: SidebarItem[]) {
  const links = getSidebarLinks(items)
