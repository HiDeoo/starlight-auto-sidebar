...
async function updateAutogeneratedGroup(
  ...
  if (segments.length === 0) {
    const definition = getAutogeneratedGroupDefinition(config, '', context)
    items = await sortAutogeneratedGroupItems(config, definition, items, config.autogenerate.directory, context)
  }
  ...
  return Promise.all(
    items.map(async (item) => {
      ...
      const definition = getAutogeneratedGroupDefinition(config, itemPath, context)

      if (item.type === 'group') {
        const label = definition?.label ?? item.label
        const entries = await sortAutogeneratedGroupItems(
          config,
          definition,
          item.entries,
          `${config.autogenerate.directory}/${itemPath}`,
          context,
        )
...
async function sortAutogeneratedGroupItems(
  ...
  const sort = definition?.sort ?? 'slug'

  const orders = await getAutogeneratedGroupItemsOrder(config, items, path, context)
  ...
  orders.sort(({ order: aOrder, path: aPath }, { order: bOrder, path: bPath }) => {
    ...
    return collator.compare(aPath, bPath) * (sort === 'reverse-slug' ? -1 : 1)
  })
...
function getAutogeneratedGroupItemsOrder(
  ...
  path: string,
  context: StarlightAutoSidebarContext,
): Promise<{ index: number; order: number; path: string }[]> {
  return Promise.all(
    items.map(async (item, index) => {
      ...
      const definition = getAutogeneratedGroupDefinition(
        config,
        stripLeadingSlash(`${path.replace(config.autogenerate.directory, '')}/${item.label}`),
        context,
      )

      return {
        index,
        order: definition?.order ?? Number.MAX_VALUE,
...
function getAutogeneratedGroupDefinition(
  config: SidebarAutogeneratedGroupConfig,
  groupPath: string,
  context: StarlightAutoSidebarContext,
): Definition | undefined {
  const dir = new URL(config.autogenerate.directory, context.contentDir)
  return context.definitions[dir.pathname]?.[groupPath]
}
